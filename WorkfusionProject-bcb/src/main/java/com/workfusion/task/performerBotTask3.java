...
public class performerBotTask3 extends baseClase implements AdHocTask {
    ...
    @Inject
    public performerBotTask3(Injector injector, Logger logger, ClientRepository cr, AccountRepository ar){
    ...
    public TaskRunnerOutput run(TaskInput taskInput) {
        ...
        rpaRunner.execute(d->{
        	...
        	if (! accountRepository.checkIfIsUpdated()) {
            	...
            	oController.crearOutput();
            	
            	// Se ejecuta el navegador hasta la pagina de inicio.
            	oController.executeBrowserTilHome();
            	
            	HashMap<String, List<Account>> cuentas = oController.obtenerClientesCuentasBD();
            	
            	// De vuelta al navegador, se actualizan los datos del cliente.
            	oController.actualizarClientesBrowser(cuentas);
            	
            	// Se actualiza el archivo de salida.
            	oController.rellenarOutput(cuentas);
        	} else {
